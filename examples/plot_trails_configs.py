...
CONFIG = (
   {
    'name': 'bandit',
    'values': ['Exp3', 'Exp3NN', 'Exp3NNUpdate', 'Exp3NNDist', 'Exp4P', 'Exp4PNN', 'random']
   },
   {
    'name': 'gamma',
    'values': [1e-5, 1e-1, 3e-1, 5e-1],
    'bandits': ['Exp3', 'Exp3NN', 'Exp3NNUpdate']
   },
   {
       'name': 'layers',
       'values': [[64,128,256], [64,128], [64]],
       'bandits': ['Exp3NN', 'Exp3NNUpdate', 'Exp3NNDist', 'Exp4PNN']
   },
   {
       'name': 'dropout_prob', 
       'values': [0.2, 0.5, 0.8],
   ...
   },
   {
       'name': 'seed', 
       'values': [42],
       'bandits': ['Exp3', 'Exp3NN', 'Exp3NNUpdate', 'Exp3NNDist', 'Exp4P', 'Exp4PNN', 'random']
   },
   {
       'name': 'use_exp3',
       'values': [True, False],
    ...
    for bandit in bandits:
        ...
        for idx, bandit_kwargs in enumerate(bandit_trials_kwargs):
            ...
            if not os.path.exists(result_json_path):
                print(f"[{i + 1}] Skipping bandit {bandit} with kwargs {bandit_kwargs}")
            
            print(f"[{i + 1}] Plotting bandit {bandit} with kwargs {bandit_kwargs}")
            with open(result_json_path, "r") as f:
                result = json.load(f)

            bandit, bandit_kwargs, bandit_regret = result['bandit'], result['kwargs'], result['regret']
            label = get_label(bandit, bandit_kwargs)
            label = f"{bandit}_{i + 1}"
            plt.plot(range(len(bandit_regret)), bandit_regret, c=col[idx % len(col)], ls='-', label=label)
            i += 1
